...
namespace ldb {
  ...
  namespace {
    std::optional<Elf64_Addr> locateLinkMap(std::istream& stream,
      ...
      for (size_t i = 0; i < ndyn; i++) {
        ...
        if (dyn.d_tag == DT_DEBUG) {
          Elf64_Addr r_debug_addr =
                  (Elf64_Addr) ptrace(PTRACE_PEEKDATA, pid, curr_addr + sizeof(dyn.d_tag), 0);
          Elf64_Addr link_map_addr = (Elf64_Addr) ptrace(
                  PTRACE_PEEKDATA, pid, r_debug_addr + offsetof(struct r_debug, r_map), 0);
    ...
    parseDynamicSymbols(std::istream& stream, const std::vector<Section>& sections, pid_t pid) {
      auto dynsec = std::find_if(sections.begin(), sections.end(), [](const Section& sec) {
        return sec.getType() == SectionType::kDynSym;
      });
      if (dynsec == sections.end()) return nullptr;

      auto link_map_addr = locateLinkMap(stream, sections, pid);
      if (not link_map_addr) return nullptr;
  ...
  std::unique_ptr<DebugInfo> ELFReader::read(const std::filesystem::path& path, pid_t pid) {
    ...
    auto string_table = parseStringTable(file_stream, header->e_shstrndx * header->e_shentsize +
                                                              header->e_shoff);
    if (not string_table) return nullptr;

    std::vector<Section> sections(parseSections(file_stream, *header, *string_table));
    if (sections.empty()) return nullptr;

    std::unique_ptr<SymbolTable> local_symbol_table = parseSymbols(file_stream, sections);
    if (not local_symbol_table) return nullptr;
  ...
  std::optional<std::string> ELFReader::parseStringTable(std::istream& istream, size_t offset) {
  ...
  std::unique_ptr<SymbolTable> ELFReader::parseSymbols(std::istream& stream,
    ...
    for (auto& sec : sections) {
      ...
      auto sym_str_table =
              parseStringTable(stream, sections[section_header.sh_link].getHdrOffset());
      if (not sym_str_table) { return nullptr; }
      ...
      for (size_t i = 0; i < n_sym; i++) {
        ...
        if (not stream.read(reinterpret_cast<char*>(&sym), sizeof(Elf64_Sym))) { return nullptr; }
        ...
        std::string name = boost::core::demangle(sym_str_table->c_str() + sym.st_name);
        std::cout << name << " " << sym.st_value << " " << sym.st_shndx << std::endl;
